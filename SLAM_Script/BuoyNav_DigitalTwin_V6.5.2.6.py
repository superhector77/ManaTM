# V6.5.2.6 # Added prop_angle calculation to thrust_angle_to_motor_servo function, using moment_cmd and moment_arm to determine the prop angle, which is then used to calculate the servo signal. This allows the motor and servo signals to be directly derived from the commanded thrust and moment, without needing a separate prop_angle input.
...
# V6.5.14: Physics & Nav Tuned (Max Thrust 30, Speed 0.5, Steer 45, No Pillars)
...
BOAT_LENGTH = 0.762       
BOAT_WIDTH = 0.2
REAR_AXIS_OFFSET = BOAT_LENGTH / 2.0   

# --- USER CONSTRAINTS APPLIED ---
MAX_SPEED = 1.0
MAX_STEER = np.radians(45) 
MAX_THRUST_LIMIT = 40.0
DT = 0.1
...
class TugboatLidar(Laser):
    def __init__(self):
        super().__init__(LIDAR_RAYS, int(1.0/DT), 360, 0)
...
class AdvancedDigitalTwin:
    def __init__(self, start_pos=(0.15*MAP_SIZE, 0.9*MAP_SIZE, -np.pi/4)):
        ...
        # 30x30m SLAM grid maintained to prevent C-Extension Localization failure!
        ...
        # --- PHYSICS TUNING: TIGHTER SLOW TURNS ---
        self.X_u, self.X_uu = 5.0, 8.0     
        self.Y_v, self.Y_vv = 10.0, 20.0   
        
        self.N_r = 1.0     # Reduced base rot drag -> Turns easily when slow
        self.N_rr = 2.0    
        self.Y_uv = 15.0   # High sliding resistance when moving fast
        self.N_ur = 4.0    # Reduced speed-coupled drag
        
        self.kp_thrust = 10.0
        self.ki_thrust = 1.0
        
        # --- PID TUNING: LESS OVERSHOOT ---
        self.kp_steer = 3.0     
        self.ki_steer = 0.05    
        self.kd_steer = 2    # Increased Derivative to strongly dampen overshoot 
    ...
    def think(self, lidar_matrix, accel_tuple, gyro_tuple):
        ...
        base_thrust = (self.kp_thrust * speed_error) + (self.ki_thrust * self.integral_speed_error)
        base_thrust = max(base_thrust, 0.0)
        
        # DYNAMIC TURN-ASSIST: Gently scaled to target_v so it doesn't plow forward.
    ...
    def _arbiter(self, desired_heading, max_speed, ignore_target=None):
        ...
        MIN_SPEED = 0.05 # Allow it to crawl if needed
        ...
        for i, r in enumerate(valid_ranges):
            if r < SAFE_DIST:
                ...
                block_width = int(4 + (SAFE_DIST - r) * 5) 
        ...
        LOOKAHEAD, CRITICAL = 4.0, 1.5
        if path_clearance >= LOOKAHEAD:
            path_speed_factor = 1.0 
        elif path_clearance <= CRITICAL:
            path_speed_factor = 0.2 
        else:
            ratio = (path_clearance - CRITICAL) / (LOOKAHEAD - CRITICAL)
            path_speed_factor = 0.2 + (0.8) * ratio
            
        # --- AGGRESSIVE TURN SLOWDOWN ---
        # If the boat is physically facing away from where it wants to go, cut speed to 10%
    ...
    def act(self, thrust_cmd, prop_angle_cmd):
        ...
        drag_Y = -(self.Y_v * self.v + self.Y_vv * self.v * abs(self.v) + self.Y_uv * abs(self.u) * self.v)
        drag_N = -(self.N_r * self.r + self.N_rr * self.r * abs(self.r) + self.N_ur * abs(self.u) * self.r)
        
        self.dot_u = (prop_force_x + drag_X + (self.mass * self.v * self.r)) / self.mass
        self.dot_v = (prop_force_y + drag_Y - (self.mass * self.u * self.r)) / self.mass
        dot_r = (generated_moment + drag_N) / self.I_z
        
        pred_u = np.clip(self.u + self.dot_u * DT, -5.0, 5.0)
        pred_v = np.clip(self.v + self.dot_v * DT, -5.0, 5.0)
        pred_r = np.clip(self.r + dot_r * DT, -5.0, 5.0)
        
        global_vx = pred_u * np.cos(self._theta) - pred_v * np.sin(self._theta)
        global_vy = pred_u * np.sin(self._theta) + pred_v * np.cos(self._theta)
        ...
        next_x = self._x + dx
        next_y = self._y + dy
        next_theta = (self._theta + dtheta + np.pi) % (2 * np.pi) - np.pi
        
        # DECOUPLED SLIDING COLLISIONS
        hit_trans, _, _ = env.check_collision(next_x, next_y, self._theta)
        if not hit_trans:
            self._x = next_x
            self._y = next_y
            self.u = pred_u
            ...
            hit_x, _, _ = env.check_collision(next_x, self._y, self._theta)
            hit_y, _, _ = env.check_collision(self._x, next_y, self._theta)
        ...
        hit_rot, _, _ = env.check_collision(self._x, self._y, next_theta)
        if not hit_rot:
            self._theta = next_theta
            self.r = pred_r
        else:
            self.r *= -0.5
            if self.collision_cooldown == 0:
        ...
        final_global_vx = self.u * np.cos(self._theta) - self.v * np.sin(self._theta)
        final_global_vy = self.u * np.sin(self._theta) + self.v * np.cos(self._theta)
        self._v = np.hypot(final_global_vx, final_global_vy)
...
def main():
    ...
    ax_rudder.set_title("Prop Angle (Rad)", fontsize=10)
    ...
    slam_img = ax2.imshow(np.zeros((boat.GRID_DIMS_SLAM, boat.GRID_DIMS_SLAM)), origin='lower', cmap='Blues', 
                          extent=[boat.slam_offset_x, boat.MAP_SIZE_SLAM + boat.slam_offset_x, 
                                  boat.slam_offset_y, boat.MAP_SIZE_SLAM + boat.slam_offset_y], 
                          vmin=0, vmax=255)
    ...
    while True:
        ...
        cmd_thrust, cmd_angle = boat.think(lidar_matrix, accel_tuple, gyro_tuple)
        boat.act(cmd_thrust, cmd_angle)

        if step % 5 == 0:   
            throttle_bar[0].set_width(boat.current_thrust)
            throttle_bar[0].set_color('red' if boat.current_thrust > 25 else 'limegreen')
            rudder_bar[0].set_width(boat.current_angle)
